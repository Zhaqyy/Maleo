...
export default function Home() {
  ...
  const MoveVidY = useTransform(scrollYProgress, [0.1, 0.2,0.99,1], ["10%", "0%", "0%", "5%"]);
  const wide = useTransform(scrollYProgress, [0.1, 0.2,0.6, 1], ["15%", "100%","100%","40%"]);
  const x = useTransform(scrollYProgress, [ 0.1, 0.2,0.6,1], ["50%","50%","50%","25%"]);
  const tall = useTransform(scrollYProgress, [0.1, 0.2, 0.59, 0.6], ["300px", "500px", "500px", "700px"]);
  const bright = useTransform(scrollYProgress, [0.1, 0.2], ["0.25", "0.75"]);
// const fil = filter=`brightness(${bright})`;
  return (
    <>
      <motion.section className="wrapper"  ref={scrollRef} style={{padding:'0'}}>
        <motion.section className="hero" ref={TargetRef}>
          ...
          <motion.video autoPlay loop muted playsInline className="hero-vid" style={{top:MoveVidY, width: wide, height:tall,left:x}}>
